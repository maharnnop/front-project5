        ...
        {isLoading ? <LoadingSpinner /> : null}
          <div className="invest-detail">
            ...
            <div className="descript">
              ...
              <h3><span>Premium : </span>{Intl.NumberFormat().format(packages.premium)}</h3>
              <h3><span>Compensate : </span>{Intl.NumberFormat().format(packages.compensation)}</h3>
              </div>
              <div className="des2">
              init_fund : {Intl.NumberFormat().format(packages.init_fund)}<br/>
              currenly : {Intl.NumberFormat().format(packages.fund)}<br/>
              </div>
              <div className="buyform">
              {localStorage.getItem("jwt") === null ? <h3>Login to buy</h3> :
              userData ? <Link to="/dashboard">already buy</Link>: buyForm}
              </div>
